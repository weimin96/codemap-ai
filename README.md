...
`pack` 会输出 Repo Map、Graph-aware Context Pack、已选文件片段和跳过原因，适合与其他 CLI 或 AI 工具串联。`--include`、`--ignore` 和 `--stdin` 只影响 pack 输出，不改变 Web UI 的项目扫描。`--include-diffs` 会附加当前 Git diff，`--include-logs` 会附加最近 20 条提交摘要。
